...
## 用户批量发布草稿时不配图，会把 MED 一次性抬成多条 HIGH（2026-08-30 新增）

**现象**：`--full` 本轮 HIGH 从 2 条涨到 5 条。新增的三条全是 `missing-image`，对应 08-29 用户一次性发布的三篇（`3-player-mahjong` / `mahjong-etiquette` / `mahjong-lessons`）。

**根因**：脚本对缺图的分级取决于发布状态 —— 草稿缺图是 MED（等配图属正常流程），已发布缺图是 HIGH（Article 结构化数据缺 `image`，Google Article 富媒体结果拿不到，og:image 退回站点 logo）。用户把草稿发布了但没走「先配图再发布」这一步，同一批文章的分级就整体跳档。**不是新的技术缺陷，是流程缺口。**

**检测方法**：`--inventory` 的 `PUB` 与 `IMG` 两列交叉看，**不需要跑 `--full`**：任何 `PUB=True` 且 `IMG=NO` 的行都是一条 HIGH。本轮 4 行命中。

**修复/防护**：
- 修复是用户侧动作（后台配封面图），**按 skill 硬规则 3 不擅自改已发布文章**。
- 防护：审阅包的「用户待办」里把配图写成**发布的前置步骤**而不是并列项，并在每轮审阅包复述当前 `PUB=True & IMG=NO` 的完整清单，让积压可见。

## 新发布文章的入站内链天然为 0，用下一篇文章顺手补掉（2026-08-30）

**现象**：08-29 新发布的三篇同时各报一条 `internal-links` MED（入站 0，目标 ≥2）。

**根因**：站上已有文章写作时它们还是草稿，按 article-spec 的内链规则**不允许链草稿**（404）。所以每一篇文章刚发布时入站必然是 0，这是规则的必然结果不是疏漏。

**检测方法**：`--full` 的 `internal-links` 条目；或直接看 `--inventory` 里最近发布的几篇。

**修复/防护**：**下一篇新文章的内链优先指向「最近发布且入站 0」的文章。** 本轮 5 个内链里 3 个用在这三篇上，一次动作把三条 MED 全部补掉，且都是自然锚点（礼仪篇接防守争议、三人篇接练习桌、找课篇接「宁可有人教」）。这条在 article-spec 里已有「优先链入站 <2 的文章」的说法，本轮确认它对**刚发布的文章**尤其有效。
