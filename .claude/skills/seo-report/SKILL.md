---
name: seo-report
description: Averill SEO 专报的分析方法论与输出规范（云端 SEO 日报/周报任务专用，v1.2）
---

# Averill SEO 专报框架 v1.2
...
## 社媒段（v1.2 新增，仅周一周报出，数据源 Instagram Graph API）

IG @averillmahjong（Business 号，token 在任务配置中，**有效期约至 10/10，10 月初日报须提醒续期**）：
- 周报报：粉丝数及周增、近 7 天触达（insights reach）、主页访问、本周新帖及其表现（赞/评）
- **停更告警**：距最近一帖超过 14 天 → 🟡"IG 停更 N 天"（背景：账号 6/25 后曾断更，而本月已有 2 单来自 IG/FB 引荐——社媒是已验证的出单渠道）
- 与订单联动：订单台账出现 instagram/facebook 引荐单时，回查当周是否有新帖/群组动作
...
纯文本单条飞书消息，末尾水印"📚 SEO框架 v1.2"（版本与本文件标题一致，不可省略）
