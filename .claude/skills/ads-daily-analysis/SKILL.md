...
description: Averill Google Ads 每日日报的分析方法论与输出规范（云端日报任务专用，v2.2）
---

# Averill Google Ads 日报分析框架 v2.2
...
- **教育系列 Education-HowToPlay-US（id 24111419407）——8/10 诊断完毕，待店主决策**：连续零投放（8/7 起）根因已查明：非资格问题（8 词全 APPROVED/ELIGIBLE、QS 6-10、广告过审、系列 SERVING），是 ¥2.0 上限仍被拍卖价顶穿（lost_rank 恒 90%+）。8/5-8/6 曾以 CPC ¥1.47 买到量的低价窗口已被 MFM 等大预算竞对关闭。选项：A. 上限再提 ¥2.5-3.0（但"低价教育流量"逻辑开始失效）；B. 暂停系列、¥40 预算转投被预算掐住的搜索系列（总预算不变）。店主决策前维持现状，日报只报状态不重复告警。
...
## 订单渠道台账（v2.2 新增，数据源：Shopify Admin API，凭据在任务配置中）
...
## SEO 监测（v2.2 新增，数据源：Google Search Console API，凭据在任务配置中）
...
## 账户改动审计（v2.2 新增，数据源：change_event）
...
▎🛒 订单（近3天新增）
#单号 | 日期 | 金额 | 渠道（归类规则见上）；无新订单写"无"
本月渠道累计：自然搜索 X 单 / 群组+直接 X / 社媒 X / 广告 X / 邮件 X
▎告警：无则"✅"
▎📚 框架 v2.2

末尾的"📚 框架 v2.2"是版本水印：证明本文件被成功读取，版本号与本文件标题一致。此行不可省略。
