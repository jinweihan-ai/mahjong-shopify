...
- **口径**：账户币种 **CNY**（cost_micros ÷ 1e6 = 人民币元，图表换算 USD 用当日汇率：汇率按当日 ECB 参考价取（GET https://api.frankfurter.dev/v1/latest?base=USD&symbols=CNY，历史日期用 /v1/YYYY-MM-DD；失败则 GET https://open.er-api.com/v6/latest/USD 取 rates.CNY；再失败用最近核对参考价兜底并在报告注明"汇率兜底"（最近核对：6.71，2026-09-09 ECB；会话里核对汇率时顺手更新此数）；**2026-09-10 实测：云端 routine 沙箱的出网策略拦截 api.frankfurter.dev 与 open.er-api.com（CONNECT tunnel 403；首尔服务器与本机均可达，不是源站问题），店主在 claude.ai 环境网络设置放行这两个域名之前，报告只能走兜底**），2026-09-09 店主指出此前写死的 7.2 与实际 6.7–6.8 不符，所有报告统一改为当日汇率并在换算处标注所用汇率），账户时区 **Asia/Shanghai**，因此 `segments.date` 天然就是北京日期，无需再换算
...
- **Charleston Garden 词组（2026-09-05 店主同意，随预售上线）**：张勇 9/5 在搜索系列 monets-US-144$ 内新建广告组「Charleston Garden」（落地页=查尔斯顿产品页，建好先暂停），9/6 启用；词：[charleston garden mahjong]、[charleston garden mahjong set]、[averill charleston]（品牌词，当前月搜索量 0，等 KOL/社媒/EDM 把名字打出去后承接）+ [charleston mahjong set]（真实购买意图，US 月量 320、CPC≈$0.95；产品名即 Charleston，相关性远高于当年泛词，作为唯一放行的非品牌词试两周）；系列层否定：账户的否定词惯例是**共享否定列表「Junk & Irrelevant - All Campaigns」（sharedSets/12172690837，已关联三条系列）**，其中已有 rules / how to play；**判断"否定词未建"前必须同时查 campaign_criterion 与该列表的 shared_criterion**（9/7 周报只查 campaign_criterion，误报"一条没建"）。店主 9/7 定「补齐否词」：待加 how to / pass / passing / what is / meaning / explained / how much（phrase，加进该共享列表）。店主 9/7 定「预算独立落地」：Charleston 4 词拆成独立搜索系列（¥40/天独立预算、尽量点击且 CPC 上限同主系列、US/英语、关联同一共享否定列表、复制现有 RSA 823625886380），主系列 monets-US-144$ 保持 ¥80/天只留品牌词+助攻双词，旧组内 Charleston 4 词暂停。**两项均已于 2026-09-08 落地（店主「你做」授权，经官方 API，先 validateOnly 再写再读回）**：共享列表新增 7 个 phrase 否词；新建搜索系列 **Charleston-US-40$（id 24231863926，预算 ¥40/天，尽量点击、CPC 上限 ¥8、US/英语、已关联共享否定列表 12172690837）**，广告组「Charleston Garden」（id 200990111178）4 个完全匹配词，RSA 823796945661 为主系列广告 823625886380 的复制（落地页查尔斯顿页，9/8 审核中）；主系列 monets-US-144$ 的 Charleston Garden 组内 4 个 Charleston 词已暂停，只留品牌词 + 助攻双词。**日报 Charleston 行改按新系列查询**：`SELECT segments.date, metrics.cost_micros, metrics.clicks, metrics.conversions, metrics.search_impression_share, metrics.search_rank_lost_impression_share FROM campaign WHERE campaign.id = 24231863926 AND segments.date DURING LAST_7_DAYS`，搜索词用 search_term_view 按 campaign.id 过滤；新系列前两日展示为 0 属审核期正常，第 3 天仍 0 → 🟡。9/13 与 9/21 两次复盘定去留。**改动审计已知项**：张勇 9/5 建组/加词/改文案与落地页、9/5 21:32 暂停「广告组 1」并把品牌词+助攻双词复制进 Charleston Garden 组（9/7 周报发现，店主已知）；**2026-09-08 经 API（client_type 为 API 类、操作者为店主授权的 Google 账号）的以下改动属已记录**：共享列表加 7 否词、新建预算与系列 Charleston-US-40$ 及其广告组/4 词/广告/地域语言/共享列表关联、主系列 4 个 Charleston 词暂停；其他改动照常。**预售期日报加一行**「Charleston 组：花费 ¥X | 点击 X | 购买 X | 顶花费词」——数据用 ad_group 维度查询（SELECT ad_group.name, segments.date, metrics.cost_micros, metrics.clicks, metrics.conversions FROM ad_group WHERE campaign.id = 23889289563 AND segments.date DURING LAST_7_DAYS）与 keyword_view 按 ad_group.name 过滤；组内出现 rules/how to 类搜索词 → 🟡 提示补否定词；[charleston mahjong set] 两周 CPA > ¥300（≈毛利 $84 折算）→ 建议暂停该词。
- **预算基线（8/16 起，防守模式）**：搜索 ¥80/天（5 词防守配置），购物已停，教育已停。2026-09-08 起 Charleston-US-40$ 独立 ¥40/天，Google 总盘 ¥120/天（主系列 ¥80 + Charleston ¥40）。Google 总盘从 ¥445/天 收缩至 ¥80/天，释放 ~¥365/天 划给 Meta Phase A（IG Boost 由同事 App 内操作，花费不在 Google 账户，日报以订单台账信号为准）。
...
▎防守词体检：品牌词 IS 与蹭量迹象、助攻双词、Charleston 4 词各自表现；Charleston-US-40$ 独立系列表现与 9/13、9/21 两次复盘（monet 双词已退役，不再体检）
