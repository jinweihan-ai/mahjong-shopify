...
description: Averill Google Ads 每日日报的分析方法论与输出规范（云端日报任务专用，v1.2）
---

# Averill Google Ads 日报分析框架 v1.2
...
- **购物系列零单判定**：7/30 起低价点击模式（CPC ~¥2）累计购买 0。阈值：累计点击 >1000 且零单 → 建议降预算 20%（¥350→¥280）。每天更新累计数。
...
末尾的"📚 框架 v1.2"是版本水印：证明本文件被成功读取，版本号与本文件标题一致。此行不可省略。
