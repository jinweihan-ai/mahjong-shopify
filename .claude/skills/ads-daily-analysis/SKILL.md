...
description: Averill Google Ads 每日日报的分析方法论与输出规范（云端日报任务专用，v2.4）
---

# Averill Google Ads 日报分析框架 v2.4
...
- **购物系列试验已提前判死并回滚（8/10，change 629505）**：8/7-8/9 展示 42/51/104 = 基线的 1-2%，零信号下"尽可能提高转化价值"无法出价（与 7/30 搜索塌陷同病理），触发预注册回滚条件。已回滚为尽量点击 + ¥4 上限。**回滚后监控（至 8/13）**：验证 CPC/点击是否回到基线（CPC ¥1.7-2.5、日点击 110-180）；未恢复 → 🔴 告警。给 Google 销售的试验复盘：小转化量账户上价值出价无信号可学，需先到 15+ 购买/月再试。原第三阶段零单判定恢复计时（购物 7/30 起累计零购买，8/17 评估是否暂停购物系列）。
- **预算基线（8/10 起）**：搜索 ¥220/天（教育系列 ¥40 转投而来，总盘不变），购物 ¥225/天，教育已暂停。预算利用率告警按新基线算。搜索加预算后监控：lost_budget 应从 33.9% 回落；若 CPC 同步上涨超 20% 说明加量买到的是更贵流量，需报告。
...
## 订单渠道台账（v2.4 新增，数据源：Shopify Admin API，凭据在任务配置中）
...
**折扣码二次归因（v2.4，优先级高于访问路径）**：订单查询须含 discountCodes。群组专属码（如 BlackGirlsMahjongToo、LADIESTHATMAHJ，特征：码名=社群名）→ 渠道直接记「群组(码名)」，即使访问路径显示 SEO/direct（社群种草后转搜索/直访是常态，码是最强证据）；AVERILLMAH 为通用码，渠道按访问路径记但备注「(码)」；无码全价单是 SEO/渠道的纯增量，单独标注「全价」。发现新码名要在日报里报出来。

**规则**：多次访问的订单报首触渠道、括号注末触；与 Google Ads 当日认领的购买交叉核对。Ads 认领但 Shopify 访问路径无广告触点时，执行**地理裁决**（v2.4）：查 user_location_view 当日转化的所在州（segments.geo_target_region + geo_target_constant 解析州名），与订单收货州（shippingAddress.provinceCode）比对——州吻合 → 该单渠道列**必须写出完整旅程「广告首触 → 自然搜索收口（跨设备·地理吻合）」**，不得只写"自然搜索"——首触是广告就要让广告出现在渠道列开头（与"多次访问报首触"同一原则）；有余力时再查 click_view（click_view.keyword_info.text + location_of_presence 城市）把首触关键词写进该行，如「广告首触("mahjong sets for sale") → 自然搜索收口」。州不吻合 → 标"⚠ 归因存疑（地理不符）"，渠道按访问路径记。先例：8/9 两笔认领转化 NC/VA 点击州与 #1045(NC)/#1046(VA) 收货州双双吻合、click_view 城市级复核命中收货城市，首触关键词分别为 "mahjong sets for sale"/"mahjong tiles set"，判为真实助攻；每天维护本月渠道累计（各渠道单数/金额）。无新订单写"无"。

## SEO 监测（v2.4 新增，数据源：Google Search Console API，凭据在任务配置中）

属性 `sc-domain:averillmahjong.com`。**GSC 数据延迟约 2 天**：取 API 返回的最近一个有数据的日期作为"SEO 最新日"，并在日报里标明该日期。

- 每日：报最新可用日的点击/展示/CTR/均位，对比前 7 天均值，±20% 才写进分析
- 每周一：加深度段——Top 10 搜索词（点击/展示/排名）及环比变化、新出现的词（新收录信号）、博客文章的搜索表现
- 与订单台账联动：出现自然搜索订单时，回查该日前后哪些词在涨，积累"什么词带来订单"
...
## 账户改动审计（v2.4 新增，数据源：change_event）
...
SEO（M/D，GSC延迟约2天）：点击 X | 展示 X | CTR X% | 均位 X.X（周一附 Top 词深度段）
...
末尾的"📚 框架 v2.4"是版本水印：证明本文件被成功读取，版本号与本文件标题一致。此行不可省略。
