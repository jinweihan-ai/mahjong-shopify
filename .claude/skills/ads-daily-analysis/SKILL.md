...
description: Averill Google Ads 每日日报的分析方法论与输出规范（云端日报任务专用，v2.1）
---

# Averill Google Ads 日报分析框架 v2.1
...
## 订单渠道台账（v2.1 新增，数据源：Shopify Admin API，凭据在任务配置中）
...
## SEO 监测（v2.1 新增，数据源：Google Search Console API，凭据在任务配置中）
...
末尾的"📚 框架 v2.1"是版本水印：证明本文件被成功读取，版本号与本文件标题一致。此行不可省略。
