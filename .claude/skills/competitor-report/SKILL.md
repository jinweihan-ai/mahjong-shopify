...
> **v1.2（2026-08-31 店主定）：取消竞品日报，只保留周日竞品周报。** 定时任务仅周日触发、一律发周报；下方"日报内容"节保留为**按需重跑专用体例**（群成员 @ 机器人随时索要竞品快照时用）。原日报的"价格地板监察"迁入周报必查项 + 按需重跑必查项。
...
## 周报内容（周日，全景；窗口上周日至本周六；v1.2 起为唯一定时产出）
