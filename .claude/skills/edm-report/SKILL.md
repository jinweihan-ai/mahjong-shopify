...
- **现役资产**：AV 欢迎序列 4 封（live，8/10 起）；AV 弃购 3 封（live）；评价请求 2 封（**live**，8/11 起，9/5 实测 7 日 20 封 / 打开 60% / 点击 25%）；Klaviyo Reviews 已嵌产品页（9/5 累计 21 条已发布 + 6 条被拒）
- **列表底数**：约 210 订阅 + 28 老客（8/11 时点）；日常新增 4-9 人/天（9/5 口径，已高于早期 3 人/天），9/4 单日 +24 为放量异常
- **Klaviyo 报表时区 = America/Los_Angeles**（GET /api/accounts/ 实测）：flow-values/flow-series 的日桶是 LA 自然日，last_7_days 返回 8 桶（7 完整日 + 当日部分）；日报「昨日」取最近一个完整 LA 日并在口径节注明，列表增长 metric-aggregates 才是 Asia/Shanghai 口径
- **待发 campaign**：AV | Launch | Charleston Garden No.8（8/31 建）、AV | Campaign | Monet's Garden 25% off（8/17 建），均 Draft 未发
