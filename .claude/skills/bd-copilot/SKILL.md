...
### /work — 今日工作台(晨报同款,周一至周六 09:30 自动推送 + 随时手动;**定时版周日静默跳过,见「自动任务·周日不发」;手动重跑不受限**)
...
**周日不发(2026-08-30 店主定,群内确认卡已点确认)**:北京时间周日的**定时运行全部静默跳过**——🌅 今日工作台(09:30)与 🌇 收盘验收(17:30)都不发总览、不发卡片、不发任何说明性消息,读完本条即结束本次运行(也不写进展日志)。判定用北京时间(UTC+8)当天星期。两个 routine 均由 http_api 创建,**agent 无权改 cron**(2026-08-30 实测 update_trigger 报 "created via http_api, agents can only update routines they created"),cron 周日照旧触发,由本条规则在运行时刹车。**只约束无 fire payload 的定时运行**:周日的群指令照常响应——/work 手动重跑、@人话问答、卡片按钮回调、确认短语,一律不受影响;周一周报也不变。想恢复周日推送,或改成「只停收盘」「周六也停」,人在群里说一句即可(改的是本条规则,不是 cron)。

- **BD 晨报(周一至周六 09:30;周日不发)**:今日到期跟进(@各负责人)/超 SLA 点名/待认领线索数/昨日漏斗 delta;全无事项发一行「BD 平稳」
- **BD 收盘(周一至周六 17:30,2026-08-29 起;周日不发)**:按 /eod 规范验收当日晨报任务,每项下结论(✅闭环/🟡有动作/🙈忽略/⏸未处理),快照进进展日志
