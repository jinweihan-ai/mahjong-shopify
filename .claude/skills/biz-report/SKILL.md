...
- **周日单位经济**：Amazon 渠道以 Amazon 日报/周报的 Finances 真实口径为准（套装单均到手 ≈$119.26，毛利 ≈$44），本报直接引用不再粗估；与独立站渠道并排对比
...
2. 本月累计：销售额 | 订单数（对照月度节奏;独立站口径,Amazon 月累计暂不并入——订单窗口仅 8 天,周日单位经济周报已并双渠道）
3. 履约：待发货订单数及最老账龄；全部已发货写"✅ 无积压"
3b. Amazon 段（按上文 Amazon 渠道规则）
4. 库存：两 SKU 海外仓可售数（预留/在途括号带上;触线才展开说;Charleston 在途 276 落仓即报）
4b. 供应链一行（2026-09-08 起：供应链日报已并入产品开发群的开品晨报，本报只给全公司看一眼，不展开）：「供应链：距 <最近开售里程碑> N 天 · 逾期 X 项」；数据=上线前任务表 板块∈{供应链,里程碑}（最近开售里程碑=板块=里程碑、任务名含「开售」、计划结束≥今天的最早一条；逾期=供应链行 状态≠已完成 且 计划结束<今天）；逾期 >0 该行 🟡；拉取失败写「供应链摘要拉取失败」不阻断
...
- **近 7 天每日营业额堆叠柱状图（全渠道,v1.6）**：x=日期（MM-DD;独立站按北京归日,Amazon 段按美西归日——2026-09-03 店主定,图题注明 "Amazon: PT days"），y=当日营业额 USD——每柱两段堆叠:独立站（#2F6B4A 底段）+ Amazon FBA（#A5731A 顶段），图例两项，柱顶标注当日总订单数；周日周报窗口放宽为近 14 天（Amazon orders 窗口仅 8 天,周报中更早日期只有独立站段并在图题注明 "Amazon: last 8 days only"）；Amazon 拉取失败则只画独立站并在卡片注明
