...
「现在在哪」里 **core→BL** 表示该口径已抽进 finance_core 并由 batch_ledger.py 调用(2026-09-15 起);只写脚本名的还在脚本里内联。
...
| A2 | 月度金额折算用月末(28 日)汇率,当月用当天;算账段用「窗口内实际汇率」= 累计净收入CNY ÷ 累计净收入USD | 2026-09-14 | RS/core→BL(mrate),SR(RATE) | 定 |
...
| # | 口径 | 定于 | 现在在哪 | 状态 |
|---|---|---|---|---|
| C1 | 投入 = 任务表按批次标签汇「实际花费」,按工序归 货款 / 包材礼盒 / 头程 / 设计打样 / 素材拍摄 / 其他;「上架销售」工序不算;大货行里的礼盒、麻将包从货款挪到包材礼盒 | 9/13、9/14 | core→BL,BM(CAT) | 定 |
| C2 | 「批次投入·补充」已退役(9/15 删表):16 行迁入 SKU其他费用,保留原类别(货款/包材礼盒/头程/设计打样/素材拍摄)进对应投入桶,不进层 3 | 店主 9/15 | core→BL,BM | 定 |
...
| C6 | 单位落地成本(批次账口径)=(货款 + 包材礼盒 + 头程)÷ 批次套数;不扣留国内与丢件;人核列可覆盖 | 9/14 | BM(单位落地成本🤖),core→BL | 定 |
