...
6. **出勤天数有新旧两套口径,算工资用「旧」(店主 2026-09-17 定)。** 拿 3 个月 18 行反推:**旧 = 应出勤天数 − 旷工 − 出差 − 请假**(15/18 严格吻合,其余 3 行差额由出差/请假解释,例如 9 月出差 3 天那位 15−3=12);**新 = 逐日按当天实际时长折算再加总,单日封顶 1 天**(证据:7 月两人实际出勤时长超过应出勤时长 213.7/224.1 vs 212,新口径仍恰好 25.000,加班不加分;另有人出勤 25 天但仅 129.9 小时,新口径给 20.5 天;小数出现 .125/.25/.5/.75 八分位)。**新口径的分子没能确证**——用「实际出勤时长 ÷ 日标准工时」拟合 18 行全不符;字段代号显示新口径(52114)与旷工天数(52115)、班内工作时长(52116)同批加入,疑似用班内工作时长,但把班内工作时长/缺勤时长打开后这两项返回空,无法验证。要坐实需拉每日明细,属打卡级数据,按隐私口径不取。**选旧的理由**:新口径会把每天早走十几分钟累积成扣半天到几天,等于把弹性工时变成计时工资;而这边缺卡次数最高 13 次、补卡最多 2 次,打卡数据本身不干净,用对时长敏感的口径算钱扣的多半是打卡疏漏。表里 `实际出勤天数` = 旧口径(工资口径),`实际出勤天数·新口径` 仅作参考,两者差多少可反映打卡纪律松紧。
7. **定时任务**(首尔 crontab,2026-09-17 装):`32 9 * * *` 刷当月;`36 9 1-8 * *` 回补上月(补卡与审批落定后数字会变)。服务器时区 UTC+8,与其他任务一致。装前的 crontab 备份在 `crontab_before_att.bak`。
