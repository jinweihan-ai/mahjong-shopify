...
- **店主:「项目表里的机器人列为什么要出现?派生数据不应该落到这里」→ 8 个 🤖 列全删,bot 不再写项目表(2026-09-16 晚)**。派生数的家是 derived 库 + GUI;团队表只在「团队要据此行动」时才留 🤖 列(任务表进度评级、报销付款状态)。项目表现在 9 列全人填。第 5 步改为:batch_master 改读项目表、派生只落 derived,下游脚本改读 derived 后退役私有 base 批次表。
