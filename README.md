...
- **FIFO 与仓库的差**:莫奈 S2 按 FIFO 剩 50 套,海外仓+FBA 实点 43 套,差的是寄样、TikTok 出货和 Amazon 还没结算的订单——批次账看回本用结算口径,看断货仍看库存现状🤖。
...
- **FIFO 剩余 < 仓库实点是正常的**:退货再入库的套数在实物里、不在 FIFO 里;看断货仍以库存现状🤖为准。
