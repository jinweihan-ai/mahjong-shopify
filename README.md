...
### 2026-09-15(二) 经营日报:莫奈补货仍是零,查尔斯顿 1.3 天见底;历史未履约单核干净了

- **莫奈的警报没人接**:9/12 报告写过「不到两天见底,在途 0」,今天(9/15)海外仓 可售仍是 **9、在途仍是 0**,`pi_update_time` 停在 9/12——**三天里仓一件莫奈都没发出去**(仓单口径近 7 天只出 10 件,而 Shopify 就下了 12 件 + TikTok 8 件),不是卖得慢,是发不出来。直接后果:#1188($136,莫奈×1)卡在 IN_PROGRESS **59 小时**,破了 48h 履约红线。Amazon 侧同步恶化:zovadros 莫奈主 SKU `TB-MDGB-2KGR` 可售从 9/12 的 17 掉到 **7**(红线 <30),在途 0,MCF 顶几天这条路也快没了。
- **查尔斯顿是另一种紧**:可售 **13**、在途 **180** 仍未落仓。9/10 落仓 13 件当天就发了 45 件(把 9/6–9/10 积压的单一次清掉),9/12 再发 24,今天(9/15)又发 13——按仓单近 7 天 70 件算 **1.3 天见底**。Amazon 侧 `AVG-CGN8-3KGR` 可售 29、在途 162、另有 91 件在 FC 之间调拨。
- **差额归因两款都拆平了**(沿用 9/12 的口径,数字都是只读现成的):莫奈 前台 70 / 仓可售 9,差 61 = 在途 0 + TikTok 36 + 寄样 26 + 残差 −1;查尔斯顿 前台 178 ≈ 可售 13 + 在途 180 − 非 Shopify 出库 14(TikTok 1 + 寄样 13) + 残差 −1。前台校准建议值:莫奈 **6**、查尔斯顿 **10**(可售 − Shopify 未履约件数),动作仍归张勇。
- **历史未履约单:虚惊,已核干净**。这次为了算日均出库把 Shopify 查询窗口从「近 25 单」拉长,翻出 6 张 PAID + UNFULFILLED 的老单(#1001/#1002/#1003/#1004/#1011/#1012,最老到 2025-12-03,账龄 6800+ 小时)。逐单查 `test` 与 `cancelledAt`:**#1001–#1004、#1011 全是 `test=True` 测试单,#1012 已于 2026-07-29 取消且金额 $0**——不构成积压,日报里注明一句就不再报。**记一条口径:履约红线点名前必须先看 `test` 与 `cancelledAt`,只按 `displayFulfillmentStatus` + 账龄会误报。**
- **Amazon Pending 单没有金额,不硬猜**:昨日 Amazon 唯一一单(麻将垫 ×1)是 `Pending`,`OrderTotal` 与 `orderItems[].ItemPrice` **都不返回**,所以昨日 Amazon 按 $0 计入并在卡片注明「待出账」。已发货单若 `OrderTotal` 为 0 则可以从 `orderItems[].ItemPrice` 补回(9/11 两单 $159.00、9/12 一单 $159.99 就是这样补上的),趋势图用的是补回后的数——**这个 fallback 值得固化进 Amazon 段**。
- 其余平稳:昨日全渠道 $737.05 / 7 单(独立站 $737.05、6 单、AOV $122.84);本月累计(9/1–9/14)独立站 $12,770.34 / 106 单,月内退款 $506.98;站点巡检首页与产品页均 200、价格串命中;供应链距 S4 全平台开售日 30 天、逾期 2 项 🟡。卡片 + 趋势图 2 条均 code=0 送达。
