...
## 2026-09-07(一) EDM 周报:首封真实群发落地($1,251/10单),存量激活里程碑关闭

- 北京 9/7 00:00(美西 9/6 09:00,正卡预售开启日)发出账号第一封真实群发 `AV | Launch | Charleston Garden No. 8 | A/B images`:425 收件 / 417 送达 / 打开 41.7% / 点击 11.3% / **10 单 $1,250.93**(客单 $125,早鸟 $129.99 口径)/ 退订 1(0.24%)/ 举报 0——8 月修好的到达率第一次变成钱;报告发出时距发送仅约 9 小时,转化仍在累积
- 自动化流 8/31-9/6 vs 8/24-8/30:收件 310 vs 275(+12.7%)、打开 51.8% vs 62.4%(-10.5pp)、点击 12.9% vs 14.0%、归因 6 单 $870.99 vs 2 单 $248(+251%)、退订 0、举报 0;弃购流人均收入 $11.45 约为 Klaviyo 2026 基准 $5.81 的两倍。打开率回落的分母是翻倍的新订阅(转盘+预售落地页),欢迎流 52.2% 仍在 40-60% 带内,非到达率复发
- 列表:周新增 100 vs 上周 63(+58.7%),9/4-9/6 连续 24/24/27,Email List 总量 479(8/11 时点约 210+28);评价 published 14 条(本周 +2)、rejected 6——首批真实评价成规模,8/13 记的「产品页 review/aggregateRating 结构化数据待验」现在可以复验了
- 三条待办里程碑:①评价请求流 live ✅、②存量激活 campaign ✅ 今日关闭、③**UTM 核查仍未做且升级为关键项**(邮件本周确定带来 $2,122,链接若无 utm_source=klaviyo 会被 GA4/经营日报算成直接流量,9/13 复盘拿不到记功);建议 9/13 开售日发第二封,受众切「9/6 未打开 + 9/7 后新增」并排除已购
- 口径与踩坑记档:Klaviyo metric-aggregates 的 filter 传朴素时间按 UTC 解释而 interval=day 的桶按 timezone 参数切,**首个桶会被截断**(实测 9/4 全天 24、以 9/4T00:00 起查只得 19),取整日必须把 start 往前放几天;flow-values-reports 支持 `timeframe:{start,end}` 自定义窗口(可带 +08:00),周环比不必用 last_7/last_30 硬凑;`GET /api/flow-messages/` 405 不可列,邮件名要走 flows/{id}/flow-actions/ → flow-actions/{id}/flow-messages/ 两跳;campaign 效果走 campaign-values-reports(A/B 只回汇总一行,读不到分版)
- SKILL edm-report 基线段按上述更新(资产/底数/里程碑/季节节点),并记下本环境无 Shopify 凭据 → 周报第 3 项「邮件归因收入占店铺总收入比」暂算不出,须显式说明不得略过
