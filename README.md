...
## 2026-09-14(一) EDM 日报:知识库两处口径过期被数据打出来(评价请求流其实早就 live)

- **日报正常发出**(卡片+图 2 条进日报群),七个 Klaviyo 接口全 200,无 🔴/🟡 告警。昨日(9/13 周日)113 封、综合打开 69.9%、点击 17.7%、退订/举报/跳出全 0、邮件归因 0 单——同日独立站整体也只有 4 单 $446.98(本周最低单量日),**零归因是低单量所致而非链路断**(9/12 尚有 1 单 $136,9/14 当日至发报时已 2 单 $249.79)。
- **口径过期①:评价请求流(XJNrZd)早已 live,不是 draft**。SKILL 背景节与待办里程碑 ① 一直写「draft,待店主开闸」,但 API 里 status=live、updated 8/11,而且**近 7 天真的在发**:23 封、打开 60.9%、带来 2 单 $221。里程碑 ① 实际在一个月前就闭环了,报告里却还在当待办每周提醒。已按 API 实况更正 SKILL 背景节。
- **口径过期②:首封 campaign 已发,里程碑 ② 不再是"待发送"**。「AV | Launch | Charleston Garden No.8 | A/B images」9/7 发出 425 收件,今天正好满 7 天结论窗口:**打开 58.8% / 点击 13.0% / 11 单 $1,367.93 / 退订 1 / 举报 0**——打开率高于任何一封 flow 邮件。但它是上新 campaign、收件 425 人(非原口径的 210+28),与「存量激活」是不是同一件事得店主定,故 SKILL 里只记事实与数据,**没有擅自把 ② 打勾**;「Monet's Garden 25% off」仍是 Draft。
- **列表增长口径也跟着抬了一个量级**:老基线「转盘约 3 人/天」已不成立,9/7–9/13 共 **205** 人(9/10 峰值 69,其后 44→30→20),是 9/7 campaign 注册潮的自然衰减,非故障;已写进 SKILL 背景节,免得下次再按 3 人/天判异常。
- **一个没报的"异动"**:分流层面只有 Review Request 触发 −39%,但样本仅 2 封,按噪声处理不作判读;真正有意义的是邮件级 **Welcome #2 昨发 53 封(7 日均 31,+71%)**——9/10 那批 69 人注册潮走到序列第 2 封,属预期内的队列推进,不是异常。
- **新增待确认**:9/12 16:08 有人建了个空壳 draft 流「Flow - Sep 12, 2026, 4:08 PM」,无任何配置,已在报告里请店主确认是在建新流还是误建。
- 教训:**背景与基线节标了「随进展更新本节」就得真更新**——这两处都不是数据问题,是报告连续若干期拿着过期的资产清单在跑,把已完成的里程碑当待办、把已抬升的基线当异常线。以后每期日报顺手拿 flows 的 status 与 campaigns 的 send 状态对一遍背景节。
