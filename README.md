...
### 2026-09-13(日) EDM 周报(窗口 9/6–9/12):首次真实 campaign 群发落地,SKILL 基线三处过时已改

- **本周是到达率验证通过后的第一次真实群发**。9/7 00:00(北京)发出「AV | Launch | Charleston Garden No.8 | A/B images」,425 收件 / 417 送达,开 58.8%、点 13.0%、**11 单 $1,367.93、人均 $3.22、转化率 2.6%**,占本周邮件归因收入的 47%。存量列表可被唤醒,这是已被测量的事实,不再是假设。
- **周总量**:发送 1,093 封(上周 312,+250%)、打开 58.1%、点击 12.3%、归因 **23 单 $2,890.43**(上周 4 单 $505.99,+471%)、退订 4、退信 13、垃圾举报 0。仅看流也在真增长:收件 668(+114%)、归因 $1,522.50(+201%)。放量后打开率只掉 1.1pt。
- **SKILL 基线三处被实测证伪,已改 v1.3 背景节**:①评价请求流 XJNrZd **早已 live**(基线仍写 draft),本周实发 23 封带回 1 单 $110.50;②里程碑②存量激活 campaign **已发生**,被更大的发布群发覆盖;③列表底数 210+28(8/11)**作废**,实发 425 人、近 15 天新增 319,底数 ≥425。**教训:基线节写着"随进展更新"却三周没人动,报告里的判分基准就会拿旧数去比。**
- **仍敞口的只剩里程碑③ UTM 核查**(utm_source=klaviyo)——且 9/10 已证实流内邮件内容不能走 API 改,必须在 Klaviyo UI 里核。
- **取数缺口(没编数)**:周报规范第 3 项「邮件归因收入 vs 店铺总收入占比」算不出——本 routine 只有 Klaviyo 只读凭据,拿不到 Shopify 总收入。卡片里明写"本期无法计算"并给了分子 $2,890.43。要自动出,得给该 routine 补一枚 Shopify 只读凭据。
- **两条建议**:①把 Draft 里挂了近一个月的「Monet's Garden 25% off」(8/17 建)排期发出去(置信度高,同列表实测人均 $3.22),**但发前必须确认莫奈库存与在途**(9/12 经营日报刚报断货);②弃购流人均 $3.29 低于 Klaviyo 2026 基准 $5.81,缺口集中在尾封 A3「Claire, reply to me」(12 收件、开 41.7%、**点击 0%**、0 单)——**样本太小现在不改**,等 n≥30 再定。
- **无红黄告警**:退订率 0.37%/送达(<0.5%)、垃圾举报 0(<0.1%)、无流打开率跌破 20%。唯一观察项:campaign 退信 1.88%(存量名单首发挤陈旧地址,属预期),**下次群发仍 >2% 就做列表清洗**。
- 杂项:9/12 16:08 有人新建了一个空白 draft flow`Flow - Sep 12, 2026, 4:08 PM`,如非有意可清掉。API 侧 flow-values-reports 自定义窗口会撞 429,retry 间隔要拉到 60s;`flows/{id}/flow-actions/?include=flow-messages` 已不支持,取邮件名要走 `flow-actions/{id}/flow-messages/`。
