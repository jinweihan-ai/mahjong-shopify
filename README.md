...
## 2026-09-07(二) 竞品周报 v1.4 首跑:卡片 2.0 不吃 `note` 标签 → 回退纯文本后修正重发,SKILL 升 v1.4.1

- 10:04 触发(9:48 那轮是旧 v1.3 结构,本轮按新 trigger 跑 v1.4)。数据当天第二次全量拉取(上一轮结果不在本容器,无法复用);sources 30 源 + products 510 SKU 落地临时文件后 python 处理
- **卡片发送失败 code=230099 / ErrCode 200861 `unsupported tag note`**:飞书卡片 2.0 已不支持 `note` 标签(v1.4 规格照经典 1.0 写的水印节)。按降级铁律先发了纯文本主报(剥 markdown、价格带每源一行)+ 分布图两条,正文必达;随后把水印元素改成 `{"tag":"markdown","text_size":"notation","content":"<font color='grey'>…</font>"}` 重发,卡片 code=0 送达。**本轮群内共 3 条(纯文本回退 + 图 + 修正后卡片)**,内容同一份
- SKILL 升 **v1.4.1**:水印规格改 markdown+notation(1.0 简卡仍用 note)、渲染边界节加这条、降级铁律措辞跟改;新增一条经验——本地校验只管 JSON 形状,管不住标签是否被 2.0 支持,**新元素类型先小卡试发**。trigger 提示词无需改(第四步只说"note 水印"以 SKILL 为准的部分已同步)
- 图表:16 源水平散点(1140×1290 px、dpi150、全英文加粗 16pt+),图例挪到左上空白带、FLOOR BREACHED 提示放坐标区外底部,避免与数据点/图例打架
- **周报结论**:🔴 价格地板仍失守——Sweet Jojo「Blue Monkey Melamine 160pc」$149.99 现货可购(预售 10 月发货),低于 $159.99 约 6%;$115 Merry mini 与 $100 Tile Haus 补充牌非同规格。活跃正装 260 款/16 源(基线 245/17),可购率 77%(基线 73%);Mahjong Loft 51→64 款仍 100% 有货、virora 可购率跌到 40% 且 min $189→$199、Mahj Tile Haus 0%→67% 回补、Maison Mahj 仍 0%。七日新品 12 款里 10 款是圣诞垫(OMM 一次 7 款 $90)——**假日礼品季从配件端开跑**;OMM/TML/MFM/Sweet Jojo 本周零折扣,🔴 头部大促未触发;打折 75 款全在 Mahjong Loft(60 款,最高 25%,常态划线价)/virora/Tile Haus
- 抓取健康 29/29 success;Mahjong Loft / Mahj Squad / Mahj Queen 最近同步 09-05 00:35Z 距今约 49h、略超 48h 上限,三家本周无新品无下架,下周若仍滞后需查
- 给店主的两条:①查尔斯顿 9/21 正价定 $189.99(置信度高,$150–210 段现在全是 melamine/mini/预售货);②10 月前上一款配件做加购比降价划算。**基线需重跑**:8/11 基线的"Averill 全场最低价"与"virora $189 = 地板"两条已失效
