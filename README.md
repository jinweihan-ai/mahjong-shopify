...
### 2026-09-09(一) EDM 日报:两个待办里程碑同时达成(评价流 live + 首次 campaign 已发),SKILL 基线更新

9/9 EDM 日报(周三=日报)拉 Klaviyo 实测,发现 SKILL 背景基线有三处已过期,本次一并更新:

- **存量激活 campaign 已发出**(里程碑②):「AV | Launch | Charleston Garden No. 8 | A/B images」9/7 00:00 北京发送,425 收件 / 417 送达,打开 56.8%、点击 12.7%、**10 单 $1,250.93**、退订 1、跳出 8、垃圾举报 0。三项跑赢行业基准,后续 campaign 以此版为模板。原基线写「待店主审批文案后发送」作废
- **评价请求流已 live**(里程碑①):flow XJNrZd 状态 live,近 7 天 20 封、打开 70%、点击 30%(全站最高);Klaviyo Reviews 累计 22 条已发布 + 6 条被拒(原基线「0 真实评价起步」作废),最近一条 9/5
- **列表底数 210 → 425+**:9/4 起单日新增 17-32 人(9/2:2、9/3:7、9/4:24、9/5:24、9/6:27、9/7:32、9/8:17,近 7 天 133 人),对比 8 月基线约 3 人/天是 6 倍,与查尔斯顿上新同期。日报已建议核增量来源(转盘/广告落地页/campaign 扩散)确认非无效注册
- 昨日(9/8)本身平静:发送 84 封(欢迎 76 + 弃购 8)、综合打开 41.7%、点击 9.5%、归因订单 0、退订/跳出/举报全 0,无 🔴🟡 告警。异动两条:欢迎流发送量 +51%(76 vs 均值 50)、打开率 −21%(40.8% vs 51.7%,放量摊薄,仍在 40-60% 基准带);弃购流发送 +37%
- 近 7 天 EDM 归因合计 **$2,232.42 / 17 单**(流 $981.49 + campaign $1,250.93),迄今最强一周;弃购流人均 $8.94 > Klaviyo 基准 $5.81
- 待办里程碑只剩 **③ 流内链接 UTM 核查**(utm_source=klaviyo,未验);草稿「AV | Campaign | Monet's Garden 25% off」(8/17 建)仍挂着,9/7 店主已定莫奈不再投放,建议归档或改写查尔斯顿款防误发
- **SKILL 改动**(edm-report v1.3):①背景与基线节按上述重写,加「首次 campaign」与「待处理草稿」两条 ②数据拉取节加日报取昨日真值的口径——last_7_days 是 7 天合计不能当昨日用,须再拉一次自定义 timeframe `{"start":"<昨日>T00:00:00+08:00","end":"<今日>T00:00:00+08:00"}`,异动判断 = 昨日值 vs(7 天合计 ÷ 7);open_rate/click_rate 为 opens÷delivered,跨邮件汇总按 delivered 加权不可直接平均;campaign-values-report 按发送日归集,发出次日起昨日窗口返回空数组属正常
- 推送:卡片 + 图(4 组柱:欢迎/弃购/评价流近 7 天 + 首个 campaign 的打开率与点击率)均 code=0。全程只读,未创建/修改/发送任何邮件、流或 campaign
